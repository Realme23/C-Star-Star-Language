...
namespace c_star_star {
    namespace pointers {
        ...
        public:
            ...
            template<typename... Q>
            constexpr static GenerationalPointer<T> MakeNewGenerationalT(uint64_t domain, Q... q...);
		...
		template<typename T>
		//Verify that the generation is valid, return bool
		constexpr bool GenerationalPointer<T>::CheckGeneration(GenerationalPointer GP) {
		...
		template<typename T>
		//Panic if the generation check does not return true
		constexpr void GenerationalPointer<T>::PanicCheckGeneration(GenerationalPointer GP) {
		...
		constexpr void GenerationalPointer<T>::DeleteNewGenerationalT(GenerationalPointer GP) {
		...
		template<typename T>
		//Try to erase if the generation checks pass, return true on success
		constexpr bool GenerationalPointer<T>::TryErase(GenerationalPointer GP) {
		...
		template<typename T>
		//Delete the raw pointer
		constexpr void GenerationalPointer<T>::DeleteRaw(GenerationalPointer GP) {
			delete GP.raw_ptr;
		...
		template<typename T>
		//Given a GenerationalPointer and a domain, insert the GenerationalPointer into the domain
		constexpr void GenerationalPointer<T>::AssociateDomain(GenerationalPointer GP, uint64_t domain) {
			PointerMaps::domains.insert({ GP.raw_ptr, domain });
		}

		template<typename T>
		//Given a GenerationalPointer and a domain, mark the pointer as associated
		constexpr void GenerationalPointer<T>::DeassociateDomain(GenerationalPointer GP) {
		...
		//Construct a new raw pointer, passing the constructor args
		template<typename... Q>
		constexpr T* GenerationalPointer<T>::getRaw(Q... q) {
